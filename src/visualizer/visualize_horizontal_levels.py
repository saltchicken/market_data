...
def main():
    ...
    if not os.path.exists(sql_file):
        logger.error(f"Could not find SQL file '{sql_file}'")
        sys.exit(1)
    ...
    logger.info(f"Fetching recent price data for {ticker}...")
    ohlc_query = text("""
    ...
    try:
        ...
        logger.error(f"Error fetching OHLC data:\n{e}")
        sys.exit(1)

    logger.info(f"Executing {sql_file} to find horizontal levels...")
    ...
    try:
        ...
        logger.error(f"Error running levels query:\n{e}")
        sys.exit(1)

    logger.info(f"Found {len(levels_df)} horizontal levels. Generating chart...\n")
