...
def main():
    ...
    for ticker, data in watch_targets.items():
        ...
        logger.info(
            f"  -> {ticker}: Prev Close={prev_close_str} | Vol={vol_str} | Buy={buy_str} | Sell={sell_str}"
        )

    if len(watch_targets) > 45:
        logger.warning(
            "WARNING: You are close to IBKR's hard limit of 50 simultaneous historical requests."
        )
    ...
    logger.info(
        "All contracts subscribed. Listening for premarket/RTH candle closes..."
    )
    ...
    try:
        while True:
            ...
            if now.time() >= datetime.time(stop_hour, stop_minute):
                logger.info(
                    f"⏰ Reached {stop_hour}:{stop_minute:02d}. Market is closed."
                )
                break
