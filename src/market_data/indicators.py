...
def run_python_indicator_pipeline(engine, target_date=None):
    ...
    if df.empty:
        logger.warning("No data found to calculate indicators.")
        return
    ...
    for w in windows:
        logger.info(f"Processing {w}d trajectories...")
        var_x = (w * (w + 1)) / 12.0
    ...
    logger.info(f"Uploading {len(final_df)} calculated records to database...")
    ...
    logger.info("Indicators successfully updated!")
